...
fun NodeData?.click(gestureClick: Boolean = false, duration: Long = 200L): Boolean {
    if (this == null || !AccessibilityServiceListener.isGranted) return false
    if (gestureClick) {
        bounds?.let {
            val x = ((it.left + it.right) / 2).toFloat()
            val y = ((it.top + it.bottom) / 2).toFloat()
            return AccessibilityServiceListener.instance!!.dispatchGesture(
...
fun NodeData?.swipe(
    ...
    if (!AccessibilityServiceListener.isGranted) return false
    return AccessibilityServiceListener.instance!!.dispatchGesture(
        GestureDescription.Builder().apply {
